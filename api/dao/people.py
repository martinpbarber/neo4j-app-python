...
class PeopleDAO:
    ...
    def all(self, q, sort = 'name', order = 'ASC', limit = 6, skip = 0):
        ...
        def get_all_people(tx, q, sort, order, limit, skip):
            ...
            # Once again neo4j dates need to be converted to strings
            cypher += """
            RETURN p {{ .*, born: toString(p.born), died: toString(p.died) }} AS person
            ORDER BY p.`{0}` {1}
            SKIP $skip
            LIMIT $limit
            """.format(sort, order)
    ...
    def find_by_id(self, id):
        ...
        def get_person(tx, id):
            ...
            row = tx.run("""
                ...
                RETURN p {
                    .*,
                    born: toString(p.born),
                    died: toString(p.died),
